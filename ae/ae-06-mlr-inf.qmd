...
Now that we have 3 or more variables, there are many ways to permute the data. We will discuss two here:
...
Now let's construct the permutation sample by permuting the single variable `species`. We do so by adding the `variables` argument to the generate function. Remove `#| eval: false` after you have filled in the code.
