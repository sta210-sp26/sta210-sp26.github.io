...
This assignment is due on **Tuesday, March 19 at 11:59pm.** To be considered on time, the following must be done by the due date:
...
-   use model conditions to evaluate assumptions.

-   use model diagnostics to identify influential points.

-   evaluate multicollinearity and consider strategies to handle it.

-   fit and interpret models with transformed variables.

# Getting started

-   Go to the [sta210-sp26](https://www.github.com/sta210-sp26) organization on GitHub. Click on the repo with the prefix **hw-03**. It contains the starter documents you need to complete the assignment.
...
a.  Fit a model using `Sex`, `Length`, `Diameter`, `Height` and `Whole_Weight` to understand variability in `Age`. Neatly display the model using 3 decimal places.
...
These exercises were motivated by Ramsey and Schafer ([2012](https://sta221-sp26.github.io/hw/hw-03-DRAFT#ref-ramsey_schafer_2012_statistical_sleuth))
...
b.  Which county appears to be an extreme outlier in the number of votes for Buchanan? Create a new data frame that doesn’t include the outlying county. **You will use this updated data frame for the remainder of this exercise and Exercise 5.**
...
Which model best fits the data? Justify your choice using appropriate statistics. **(Note: Use the data set without the outlying county to find the candidate models.)**
