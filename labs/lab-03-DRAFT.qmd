...
a.  Fit a model using `sugarpercent`, `pricepercent`, `chocolate`, `peanutalmondy`, the interaction between `pricepercent` and `chocolate`, and the interaction between `chocolate` and `peanutalmondy` to predict `winpercent`. Neatly display the model using 3 decimal places. *Remember to use the variables created in the previous exercise.*
...
Let's consider another model. Fit a model that includes `chocolate`, `pricepercent`, `crispedricewafer`, `peanutalmondy`, `sugarpercent` , along with the interaction between `pricepercent` and `peanutalmondy` . Neatly display the model using 3 decimal places. *Remember to use the variables created in Exercise 2.*

## Exercise 6

a.  Compute the RMSE for the models fit in [Exercise 3] and [Exercise 5].
...
-   Select all team members’ names, so they receive credit on the assignment. [Click here](https://help.gradescope.com/article/m5qz2xsnjy-student-add-group-members) for instructions on adding team members to assignment on Gradescope.
