...
-   $SE_{\hat{\beta}_j} = \sqrt{Var_{\hat{\beta}_j}}$. Let's take a look at the variance-covariance matrix for a the model $\widehat{\text{body_mass_g}} = \hat{\beta}_0 + \hat{\beta}_1 \text{flipper_length_mm} + \hat{\beta}_2 \text{bill_length_mm}$

-   $Var_{\hat{\beta}_{flipper}, \ldots, \hat{\beta}_{bill}} =$
