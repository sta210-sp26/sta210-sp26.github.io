...
-   $SE_{\hat{\beta}_j} = \sqrt{Var_{\hat{\beta}_j}}$. Let's take a look at the variance-covariance matrix for a model with flipper length and bill length
...
round(glance(penguin_int_fit)$sigma * solve(t(X)%*%X), 3)
```
...
2.  **Calculate test statistic and p-value:** The test statistic is $t= 32.722$. The p-value is calculated using a $t$ distribution with `r nrow(penguins)` degrees of freedom. The p-value is $\approx 0$ .
...
```{r}
tidy(penguin_int_fit, conf.int = TRUE, conf.level = 0.95) |>
  kable(digits= 3)
```
...
tidy(penguin_int_fit, conf.int = TRUE) |>
  kable(digits = 3)
```

<br>

We are 95% confident that for every one millimeter increase in a penguin's flipper length, its weight is greater by 21.017 to 33.509 grams, on average, holding bill length and species constant.
