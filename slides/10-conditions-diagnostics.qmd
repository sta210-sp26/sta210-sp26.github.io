...
format: 
  ...
  html: 
    output-file: 10-conditions-diagnostics-notes.html
html-math-method:
  method: mathjax
...
-   [Lecture recordings](https://duke.hosted.panopto.com/Panopto/Pages/Sessions/List.aspx?folderID=edec3a44-0ec4-4414-89b5-b3c901375980)
...
```{=html}
<center><iframe width="640px" height="480px" src="https://forms.office.com/r/LBXenrfGJw?embed=true" frameborder="0" marginwidth="0" marginheight="0" style="border: none; max-width:100%; max-height:100vh" allowfullscreen webkitallowfullscreen mozallowfullscreen msallowfullscreen> </iframe></center>
```
:::::

## Permutation illustrated

![](images/10/permutation-sampling.png){fig-align="center" width="80%"}

<small>Generate from Claude AI Sonnet 4.5 using the prompt: "Please draw a picture illustrating permuting the response variable versus permuting individual predictor variables."</small>
...
<center>Distribution approximately unimodal and symmetric, aside from the outlier. There are `r nrow(lemurs)` observations, so inference robust to departures from normality</center>
...
resid_taxon <- ggplot(data = lemurs_aug, aes(x = taxon, y = .resid)) + 
  geom_boxplot(fill = "steelblue", color = "black") + 
  geom_hline(yintercept = 0, color = "red", linetype = 2) + 
  labs(x = "Taxon", 
       y = "Residuals", 
...
<center>**The independence condition is** **satisfied**.</center>

## What conditions are needed? {.midi}
